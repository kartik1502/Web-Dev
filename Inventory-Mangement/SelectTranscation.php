...
<head>
    ...
    <title>Transcation | Inventory Management</title>
...
. mysqli_connect_error());
        ...
        if(mysqli_num_rows($result)<=0)
        {
            ...
            {
                self.location="AddTranscation.php";
            }
            </script> ';
            exit();
        }
            ...
            echo "<center>";
                ...
                echo '<h1 style="font-size: 40px">Transcation Details</h1>';
                echo '</form>';
                echo '<div class="table-wrapper">';
                echo '<table class="fl-table">';
                echo '<thread><tr><th>Transcation ID</th><th>Order ID</th><th>Date of Transcation</th><th>Total Amount</th><th>Amount Paid</th></tr></thread>'."<br>";
                while($row=mysqli_fetch_assoc($result))
                {
                    echo "<tr><td>".$row['Trans_id']."</td><td>".$row['Ord_id']."</td><td>".$row['Date_trans']."</td><td>".$row['Tot_Amt']."</td><td>" .$row['Amount_Paid']. "</td></tr>";
                }
                echo "</table>";
                echo "</div>";
                echo "</center>";
