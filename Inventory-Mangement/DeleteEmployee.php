	  ...
	  <a href="javascript:void(0);" style="font-size:15px;" class="icon" onclick="myFunction()">&#9776;</a>
        ...
        <form class="forms1" method=POST>
            ...
            <?php
              ...
              mysqli_close($conn);
            ?><br><br>
            <input type="submit" name="" class="submit-btn0">
            </div>
        </form>
    </div>
...
<?php
    ...
    if(isset($_REQUEST['Emp_id'])){
        ...
        if(mysqli_num_rows($result)<=0)
        {
            ...
            if(confirm("Employee Details not found!!!!!!!"))
            {
                self.location="Employee.html";
            }
            </script> ';
            exit();
        }
        else{
            $sql = "DELETE FROM AddEmployee where Emp_id = '$Emp_id'";
            if(mysqli_query($conn, $sql)){
                echo '<script>alert("Employee details Deleted Sucessfully")</script>';
            }
            else{
