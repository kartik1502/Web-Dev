...
. mysqli_connect_error());
        ...
        if(mysqli_num_rows($result)<=0)
        {
            ...
            echo "<center>";
                ...
                echo '<h1 style="font-size: 40px">Stocks Details</h1>';
                echo '</form>';
                echo '<div class="table-wrapper">';
                echo '<table class="fl-table">';
                echo '<thread><tr><th>Product ID</th><th>Product Name</th><th>Date Received</th><th>Serial Number</th><th>Brand Name</th><th>Avaliability</th></tr></thread>'."<br>";
                while($row=mysqli_fetch_assoc($result))
                {
                    echo "<tr><td>".$row['Prod_id']."</td><td>".$row['Prod_Name']."</td><td>".$row['Date_recv']."</td><td>".$row['Ser_No']."</td><td>" .$row['Brnd_Name']. "</td><td>".$row['Avaliability']. "</td></tr>";
                }
                echo "</table>";
                echo "</div>";
                echo "</center>";
